    ...
    <div className="space-y-6 px-2 sm:px-0">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="px-4 py-2 bg-primary-50 rounded-lg border border-primary-200">
            ...
            <p className="text-2xl font-bold text-primary-600">{policies?.length || 0}</p>
          </div>
          <div className="px-4 py-2 bg-green-50 rounded-lg border border-green-200">
            <p className="text-xs text-gray-600">Total Coverage</p>
            <p className="text-2xl font-bold text-green-600">${(totalCoverage / 1000).toFixed(0)}K</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {filteredPolicies?.map((policy) => (
          <div
            key={policy.id}
            className="group relative overflow-hidden rounded-2xl bg-white border-2 border-gray-200 hover:border-primary-400 hover:shadow-2xl transition-all duration-300 cursor-pointer"
          >
            {/* Gradient Header */}
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <p className="font-mono font-bold text-gray-900">{policy.policy_number}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-bold text-primary-600">${policy.sum_insured?.toLocaleString()}</p>
                </div>
                <div>
                  <div className="flex items-center text-sm text-gray-500 mb-1">
              ...
              <div className="pt-4 border-t border-gray-200">
                <button className="w-full flex items-center justify-center gap-2 py-2 px-4 bg-gradient-to-r from-primary-600 to-purple-600 text-white rounded-lg font-medium hover:shadow-lg transition-all group-hover:scale-105">
                  View Details
